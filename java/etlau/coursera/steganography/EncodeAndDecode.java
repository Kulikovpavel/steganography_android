...
public class EncodeAndDecode{
    ...
    public static TwoReturn encodePicture(String hideThis,String hideIn,String writeTo){
        ...
        placeToHide = placeToHide.copy(Bitmap.Config.ARGB_8888, true);
        /**
        ...
        for(int j = 0; j < placeWidth; j += 1){
            ...
            for (int i = 0; i < placeHeight; i += 1){
                ...
                if((i == 0) && (j == 0)){
                    ...
                    placeToHide.setPixel(j,i,changeColor(placeToHide.getPixel(j,i),999));
                                        /* Places 423 in the current pixel of the
                                           image to indicate the end of the hidden image
                                           has been reached. + 2 accounts for the dimensions
                                           of the hidden image. */
                    try{
                        FileOutputStream output = new FileOutputStream(writeTo, false);
...
//                    hiddenPixels = new int[3 * hiddenWidth * hiddenHeight];
                    ...
                    if (pixelInt == 999){
                        ...
                        hiddenPixels[pixelIndex] = pixelInt;
                        pixelIndex++;
    ...
    public static Bitmap rebuildImage(int[] pixels,int width,int height,String writeTo){
        ...
        for (int i = 0; i < color.length; i += 1){
            color[i] = Color.argb(255, pixels[(3 * i)],pixels[(3 * i) + 1],pixels[(3 * i) + 2]);
        }

        int[] rotated_color = new int[pixels.length/3];

        int counter = 0;
        for (int i = 0; i < height; i++)
            for (int j = 0; j < width; j++){
                rotated_color[counter] = color[j*height + i];
                counter++;
        }
        /**
         * Hidden image.
         */
        Bitmap hiddenImage = Bitmap.createBitmap(rotated_color,width,height,Bitmap.Config.ARGB_8888);
        try{
            FileOutputStream output = new FileOutputStream(writeTo);
...
// RECREATE THE NEW BITMAP
        ...
        Bitmap resizedBitmap = Bitmap.createBitmap(original, 0, 0, original.getWidth(), original.getHeight(), matrix, true);
    ...
    public static int changeColor(int currentColor,int inputNumber){
        ...
        return Color.argb(255, digitsToInt(red), digitsToInt(green),
                digitsToInt(blue));
